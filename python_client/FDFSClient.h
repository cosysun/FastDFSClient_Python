...
public:
    ...
    // 上传普通文件
    int upload_file(const char *file_buff, const int64_t file_size,
            const char *file_ext_name, char *&remote_file_id, int &name_size);

    // slave文件上传
    int upload_slave(const char *file_buff, const int64_t file_size,
            const char *master_file_id, const char *prefix_name, const char *file_ext_name,
            char *&remote_file_id, int &name_size);

    // 上传追加类型文件
    int upload_appender(const char *file_buff, const int64_t file_size,
            const char *file_ext_name, char *&remote_file_id, int &name_size);
    ...
    // 下载文件
    int download_file(const char *remote_file_id, BufferInfo *pBuff);

    // 删除
    int delete_file(const char *remote_file_id);

    // 获取文件信息
    int get_file_info(const char *remote_file_id, BufferInfo *file_info);
